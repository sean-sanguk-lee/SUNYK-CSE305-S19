...
CREATE TRIGGER update_matched_in_waitlist
    ...
    FOR EACH ROW
BEGIN
    SET @qid = (SELECT MAX(qid) FROM ORDER_QUEUE),
        @min_price = (SELECT min_price FROM RESTAURANT r
                                    WHERE r.rid = (SELECT rid FROM WAITLIST w WHERE w.qid = @qid)),
        @w_psum = (SELECT SUM(PRICE) FROM MENU m, (SELECT mid FROM ORDERS o, (SELECT oid FROM ORDER_QUEUE oq WHERE @qid = oq.qid)t
                                                                WHERE o.oid = t.oid)z
                                                                    WHERE m.mid = z.mid);

        IF @min_price <= @w_psum THEN
            UPDATE WAITLIST w SET matched = TRUE WHERE w.qid = @qid;
        END IF;
...
INSERT INTO MENU (menu, price, splittable) VALUES ("Kimchi Fried Rice", 6500, TRUE);                 # 6
...
INSERT INTO MENU (menu, price, splittable) VALUES ("Sashimi Set A", 40000, TRUE);
...
INSERT INTO MENU (menu, price, splittable) VALUES ("Men's Pizza", 17700, FALSE);
...
INSERT INTO MENU (menu, price, splittable) VALUES ("Boneless Fried Chicken", 18000, TRUE);
INSERT INTO MENU (menu, price, splittable) VALUES ("Boneless Spicy Chicken", 19000, TRUE);
...
INSERT INTO MENU (menu, price, splittable) VALUES ("Fire Chicken", 17000, TRuE);
